    ...
    public class CustomerController : Controller
    {
...

        {
            if (ModelState.IsValid)
            {
                ...
                var currentUser = context.Customers.Where(t => t.EmailId.ToLower().Equals(user.EmailId.ToLower()) && t.Password == user.Password).FirstOrDefault();
        ...
        [HttpGet]
        public ActionResult SavedCustomerForm(String id)
        {
            Customer c = new Customer();
            c = context.Customers.Where(t => t.CustomerId == id).FirstOrDefault();
            Loan loanc = context.Loans.Where(t => t.CustomerId == id && t.status=="Saved").FirstOrDefault();
            CustomerFormViewModel u = new CustomerFormViewModel();
            u.CustomerId = c.CustomerId;
            u.CustomerName = c.CustomerName;
            u.EmailId = c.EmailId;
        ...
        public ActionResult SavedCustomerForm(CustomerFormViewModel cust, string command)
        {
            if (ModelState.Count > 10)
            {
                ...
                c.Aadhar_number = cust.Aadhar_number;
                c.Pan_number = cust.Pan_number;
                Loan l = context.Loans.Where(t => t.CustomerId == cust.CustomerId && t.status == "Saved").FirstOrDefault();
                if (cust.CollateralType == null)
                {
                    ModelState.AddModelError("", "Enter the Collateral details");
                    return View();
        ...
        [HttpGet]
        public ActionResult SubmittedCustomerForm(String id)
        {
            SaveFormViewModel s = new SaveFormViewModel();
            Customer c = new Customer();
            c = context.Customers.Where(t => t.CustomerId == id).FirstOrDefault();
            Loan l = new Loan();
            l = context.Loans.Where(t => t.CustomerId == id && t.status=="Submitted").FirstOrDefault();
            s.customer = c;
            s.loan = l;
        ...
        public ActionResult Details(int id)
        {
            ...
            if(l.status=="Saved")
            {
                return RedirectToAction("SavedCustomerForm",new { id = l.CustomerId });
            }
            else if(l.status=="Submitted")
            {
                return RedirectToAction("SubmittedCustomerForm",new { id = l.CustomerId });
            }
